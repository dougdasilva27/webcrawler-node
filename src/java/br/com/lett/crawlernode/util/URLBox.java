...
public class URLBox {
  ...
  public static String takeAScreenShot(String url, Session session, int page, List<Cookie> cookies) {
    ...
    String dateTime = new DateTime(DateTimeZone.forID("America/Sao_Paulo")).toString("yyyy-MM-dd hh:mm:ss");
    ...
    if (session instanceof RankingSession) {
      String path = "%2F" + pathRanking + "%2F" + ((RankingSession) session).getLocation().replace(" ", "_") + "%2F" + session.getMarket().getName()
          + "%2F" + date + "%2F" + page + "-" + hash;

      urlBuilder.append(path);
      options.put("s3_path", path);
    } else if (session instanceof TestRankingSession) {
      String path = "%2F" + pathRanking + "%2F" + ((TestRankingSession) session).getLocation().replace(" ", "_") + "%2F"
          + session.getMarket().getName() + "%2F" + date + "%2F" + page + "-" + hash;

      urlBuilder.append(path);
      options.put("s3_path", path);
    } else {
      String path = "%2Fblackfriday%2F" + session.getProcessedId() + "%2F" + dateTime.replace(" ", "%20").replace(":", "%3A");
  ...
  public static String getUrlForHtmlUrlBox(String url, Session session, int page, List<Cookie> cookies) {
     ...
     * options.put("proxy", proxy.getUser() + "%3A" + proxy.getPass() + "%40"+ proxy.getAddress()+ "%3A"
     * + proxy.getPort()); } else if( proxy.getAddress() != null && proxy.getPort() != null) {
     * options.put("proxy", proxy.getAddress() + "%3A" + proxy.getPort()); } }
     */
    ...
    if (session instanceof RankingSession) {
      ...
      String path = "%2Fteste%2F" + session.getProcessedId() + "%2F" + dateTime.replace(" ", "---").replace(":", "-");
  ...
  public static String generateUrl(String url, Map<String, Object> options, String key, String secret, List<Cookie> cookies, Session session)
  ...
  private static String generateToken(String input, String key, Session session) {
    ...
    try {
      ...
      Logging.printLogWarn(logger, session, "Problems calculating HMAC");
      Logging.printLogWarn(logger, session, CommonMethods.getStackTrace(lEx));
    }
