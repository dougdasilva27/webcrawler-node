...
public class BrasilEletrozemaCrawler extends CrawlerRankingKeywords {
  ...
  private String crawlProductUrl(JSONObject sku) {
    ...
    if (sku.has("attributes")) {
      ...
      if (attributes.has("product.route")) {
        ...
        if (!route.isEmpty()) {
          productUrl = CrawlerUtils.completeUrl(route, "https:", "www.zema.com");
        }
      }
    }
  ...
  private JSONObject fetchJsonApi() {
    ...
    String url = "https://www.zema.com/ccstoreui/v1/search?Ntt=" + this.keywordWithoutAccents.replace(" ", "%20") + "&No=" + this.arrayProducts.size()
        + "&Nrpp=50";
