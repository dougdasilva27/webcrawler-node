...
public abstract class BrasilVilanova extends CrawlerRankingKeywords {
  ...
  protected void processBeforeFetch() {
    ...
    Request request = RequestBuilder.create().setUrl(LOGIN_URL).setPayload(payloadString)
        .setHeaders(headers).build();
    List<Cookie> cookiesResponse = this.dataFetcher.post(session, request).getCookies();

    for (Cookie cookieResponse : cookiesResponse) {
      BasicClientCookie cookie = new BasicClientCookie(cookieResponse.getName(),
          cookieResponse.getValue());
  ...
  protected void extractProductsFromCurrentPage() {
    ...
    this.pageSize = 24;
    String url = "https://www.vilanova.com.br/Busca/Resultado/?p=" + this.currentPage + "&loja=&q="
        + this.keywordEncoded
    ...
    if (!products.isEmpty()) {
      ...
      for (Element e : products) {
        ...
        String productUrl = CrawlerUtils
            .scrapUrl(e, ".img-name a", "href", "https", "www.vilanova.com.br");
  ...
  protected void setTotalProducts() {
    this.totalProducts = CrawlerUtils
        .scrapIntegerFromHtml(this.currentDoc, ".qtd-produtos", true, 0);
