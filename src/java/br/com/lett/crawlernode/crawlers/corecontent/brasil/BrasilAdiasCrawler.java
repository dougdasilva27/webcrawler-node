 ...
 * 
 * 1) For this crawler, we have one url per each sku. There is no page is more
 * than one sku in it.
 * 
 * 2) There is no stock information for skus in this ecommerce by the time this
 * crawler was made.
 * 
 * 3) There is no marketplace in this ecommerce by the time this crawler was
 * made.
 * 
 ...
 * 
 * 6) There is no internalPid for skus in this ecommerce. The internalPid must
 * be a number that is the same for all the variations of a given sku.
 * 
 * 7) We have one method for each type of information for a sku (please carry on
 * with this pattern).
 * 
...
public class BrasilAdiasCrawler extends Crawler {
	...
	private Prices crawlPrices(Document doc, String internalId) {
		...
		if (bankTicketPriceElement != null) {
			bankTicketPrice = Float.parseFloat(bankTicketPriceElement.text().trim().replaceAll("[^0-9,]+", "")
					.replaceAll("\\.", "").replaceAll(",", "."));
		}

		// crawl the card payment options
		Map<Integer, Float> installments = new TreeMap<Integer, Float>();
		Elements installmentsElements = doc
				.select("#produto-pagamentoparcelamento-" + internalId + " .details-content p");
		if (installmentsElements.size() > 0) {
			for (Element installmentElement : installmentsElements) {
				...
				if (installmentValueElement != null) {
					installmentValue = Float.parseFloat(installmentValueElement.text().trim().replaceAll("[^0-9,]+", "")
							.replaceAll("\\.", "").replaceAll(",", "."));
				}
	...
	private String crawlSecondaryImages(Document document) {
		...
		for (int i = 1; i < imagesElement.size(); i++) { // starting from index
															// 1, because the
															// first is the
															// primary image
