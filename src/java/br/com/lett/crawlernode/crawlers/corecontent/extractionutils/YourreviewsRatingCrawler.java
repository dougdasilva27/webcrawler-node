...
public class YourreviewsRatingCrawler {
   ...
   private boolean hasNextPage(Document docRating, Integer currentPage) {
      ...
      Elements pages = docRating.select(".yv-footer-paging:not([style]) .yv-paging:not(:last-child)");

      if (!pages.isEmpty() && !pages.last().text().replaceAll("[^0-9]", "").equals(currentPage.toString())) {
         hasNextPage = true;
      }
