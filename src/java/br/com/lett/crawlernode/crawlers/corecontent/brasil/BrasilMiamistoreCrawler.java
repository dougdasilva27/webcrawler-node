...
public class BrasilMiamistoreCrawler extends Crawler {
    ...
    if (isProductPage(doc)) {
      ...
      for (int i = indexStart; i < indexFinished; i++) {
        ...
        Product product = ProductBuilder.create().setUrl(session.getOriginalURL()).setInternalId(internalId).setInternalPid(internalPid).setName(name)
            ...
            .setCategory3(categories.getCategory(2)).setPrimaryImage(primaryImageVariation).setSecondaryImages(secondaryImagesVariation)
      ...
      if (elementFotoSecundaria.size() > 1) {
        for (int i = 1; i < elementFotoSecundaria.size(); i++) { // starts with index 1 because de primary image is the first image
          ...
          if (e != null) {
            ...
            if (!primaryImage.equals(secondaryImagesTemp) && !secondaryImagesList.contains(secondaryImagesTemp)) { // identify if the image is the
                                                                                                                   // primary image
  ...
  private String crawlDescription(Document document) {
    String description = "";
    Element elementProductDetails = document.select(".wd-descriptions").first();
