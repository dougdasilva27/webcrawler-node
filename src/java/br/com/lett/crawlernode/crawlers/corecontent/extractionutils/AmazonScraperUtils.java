...
public class AmazonScraperUtils {
  ...
  public Document fetchProductPage(List<Cookie> cookies, DataFetcher dataFetcher) {
    Document doc = Jsoup.parse(fetchPage(session.getOriginalURL(), new HashMap<>(), cookies, session, dataFetcher));
    ...
    if (doc.selectFirst("#captchacharacters") != null) {
      ...
      doc = Jsoup.parse(fetchPage(session.getOriginalURL(), new HashMap<>(), cookies, session, dataFetcher));
    }
   ...
   * @return
   */
  public String fetchPage(String url, Map<String, String> headers, List<Cookie> cookies, Session session, DataFetcher dataFetcher) {
    String content;
