 ...
 * 
 * 1) For this crawler, we have multiple skus on the same page. In cases we have color selector, we have one URL per sku.
 ...
 * 1) We have two distinct methods to crawl the payment options. One is used in cases where we don't have
 * sku variations on the same page, and the other is used when we have multiple variations on a page. These
 * variations doesn't include colors variations, because in these cases we have one URL per sku.
 * 2) When crawling prices for multiple variations we use an API response, but for no variations we simply crawl the data
 * from the html.
 * 3) To crawl the installments prices when fetching data from API, we must compute each installment value by dividing
 * the base price for a installment number.
 * 
 * Examples:
...
public class BrasilEcontinentalCrawler extends Crawler {
	...
	/**
	 * Fetch an API response to crawl price information.
	 * This response is used to get the normal price and all the payment options as well.
	 * 
	 * @param internalID
	 * @return
	 */
