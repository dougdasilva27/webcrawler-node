...
public class AustraliaCatchCrawler extends Crawler {
    ...
    public List<Product> extractInformation(Document doc) throws Exception {
        ...
        if (isProductPage(doc)) {
            ...
            Map<String, String> skus = scrapSkuVariations(variation, internalId);
    ...
    private Map<String, String> scrapSkuVariations(Elements doc, String internalId) {
        ...
        for (Element element : doc.select(".attribute-dropdown__option")) {
            ...
            if (sufixoElem != null)
                if (sufixoElem.text() != null && element.select(".attribute-description").isEmpty()) {
    ...
    private String crawlInternalId(Element doc) {
        ...
        for (Element elem : scripts) {
            String script = elem.toString().replace(" ", "");
            if (script.contains("sku_id")) {
                String skuIdSubString = script.substring(script.indexOf("sku_id"));
                internalId = Objects.requireNonNull(MathUtils.parseInt(skuIdSubString.substring(0, skuIdSubString.indexOf(",")))).toString();
            }
        }
