...
public class ArgentinaLaanonimaonlineCrawler extends Crawler {
   ...
   private Double scrapAvgRating(Document doc) {
      double avg = 0d;
      avg = CrawlerUtils.extractRatingAverageFromAdvancedRatingReview(getTotalAvgRating(doc));
      return avg;
   }
   ...
   private AdvancedRatingReview getTotalAvgRating(Document doc) {
      ...
      Elements reviews = doc.select("#ver_comentarios .contenedor.clearfix");

      for (Element review : reviews) {

         Element one = review.select("span[title=\"No recomendado\"].activa").first();
         Element two = review.select("span[title=\"Malo\"].activa").first();
         Element tree = review.select("span[title=\"Regular\"].activa").first();
         Element four = review.select("span[title=\"Bueno\"].activa").first();
         Element five = review.select("span[title=\"Muy Bueno\"].activa").first();
