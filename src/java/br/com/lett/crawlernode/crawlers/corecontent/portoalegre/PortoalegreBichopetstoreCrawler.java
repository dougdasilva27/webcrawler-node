...
public class PortoalegreBichopetstoreCrawler extends Crawler {
  ...
  public List<Product> extractInformation(Document doc) throws Exception {
    ...
    if (isProductPage(doc)) {
      ...
      String internalId = CrawlerUtils.scrapStringSimpleInfoByAttribute(doc, "#product > .form-group [name=\"product_id\"]", "value");
      String internalPid = CrawlerUtils.scrapStringSimpleInfoByAttribute(doc, "#product > .form-group [name=\"product_id\"]", "value");
      ...
      if(!variations.isEmpty()) {
        for(Element variation : variations) {
          ...
          Prices variationPrices = scrapVariationPrices(doc, variationId);
  ...
  private Prices scrapVariationPrices(Document doc, String variationId) {
    Prices prices = new Prices();
    Map<Integer, Float> installmentPriceMap = new HashMap<>();
    
    Elements installments = doc.select("#product .parcelamento [id=\"" + variationId + "\"] table tr > td > strong");
    for(Element e : installments) {
      Pair<Integer, Float> installment = CrawlerUtils.crawlSimpleInstallment(null, e, true, "x de", "", true);
