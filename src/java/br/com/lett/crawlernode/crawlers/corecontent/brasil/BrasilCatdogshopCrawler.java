...
public class BrasilCatdogshopCrawler extends Crawler {
  ...
  public List<Product> extractInformation(Document doc) throws Exception {
    ...
    if (isProductPage(doc)) {
      ...
      for (int i = 0; i < jsonArr.length(); i++) {
        ...
        String internalId =
            skuJson.has("product_id") && !skuJson.isNull("product_id")
                ? skuJson.get("product_id").toString()
                : null;
        String internalPid =
            skuJson.has("sku") && !skuJson.isNull("sku") ? skuJson.get("sku").toString() : null;
        String name =
            CrawlerUtils.scrapStringSimpleInfo(doc, ".product-form-container .product-name", true);
        CategoryCollection categories =
            CrawlerUtils.crawlCategories(doc, ".breadcrumbs > a.breadcrumb-crumb", true);
        String primaryImage =
            CrawlerUtils.scrapSimplePrimaryImage(
                doc,
                "#product-slider-container .product-slide > a",
                Arrays.asList("href"),
                "https:",
        ...
        Integer stock =
            skuJson.has("stock") && skuJson.get("stock") instanceof Integer
                ? skuJson.getInt("stock")
                : null;
  ...
  private Offers scrapOffers(Document doc, JSONObject json)
      throws OfferException, MalformedPricingException {
    ...
    if (spotlightPrice != null) {
      Double priceFrom =
          CrawlerUtils.scrapDoublePriceFromHtml(
              doc, "#compare_price_display", null, false, ',', session);
      CreditCards creditCards = scrapCreditCards(json);
      ...
      return PricingBuilder.create()
          ...
          .setBankSlip(
              BankSlipBuilder.create().setFinalPrice(spotlightPrice).setOnPageDiscount(0d).build())
          .build();
    }
  ...
  private CreditCards scrapCreditCards(JSONObject skuJson) throws MalformedPricingException {
    CreditCards creditCards = new CreditCards();

    Installments installments = new Installments();
    if (skuJson.has("installments_data")
        && !skuJson.isNull("installments_data")
        && skuJson.get("installments_data") instanceof String) {
      ...
      for (String key : installmentsJson.keySet()) {
        ...
        JSONObject installmentJson =
            installmentsJson.get(key) instanceof JSONObject
                ? installmentsJson.getJSONObject(key)
                : new JSONObject();
        Double second =
            JSONUtils.getDoubleValueFromJSON(installmentJson, "installment_value", true);
