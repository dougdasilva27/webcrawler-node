 ...
 *
 */
public class EspanaAmazonCrawler extends Crawler {

   private static final String PROTOCOL = "https";
   private static final String HOST = "www.amazon.es";
   private static final String HOME_PAGE = PROTOCOL + "://" + HOST;
   private static final String SELLER_NAME = "amazon.es";
   private static final String SELLER_NAME_2 = "amazon.com";
   private static final String SELLER_NAME_3 = "Amazon";

   private static final String SELLERS_URL_TOKEN = "offer-listing/";
   ...
   private List<Document> fetchDocumentsOffers(Document doc, String internalId) {
      ...
      Element marketplaceUrl = doc.selectFirst("#buybox-see-all-buying-choices-announce, #olpLinkWidget_feature_div a");

      if (marketplaceUrl != null) {
         String urlMarketPlace = setIdOnSellersUrl(CrawlerUtils.completeUrl(marketplaceUrl.attr("href"), PROTOCOL, HOST), internalId);
         ...
         Element nextPage = docMarketplace.selectFirst(".a-last:not(.a-disabled) a");
         int page = 1;

         while (nextPage != null && page < 20) {
            String nextUrl = CrawlerUtils.completeUrl(nextPage.attr("href"), PROTOCOL, HOST);
            ...
            nextPage = nextDocMarketPlace.selectFirst(".a-last:not(.a-disabled) a");
            headers.put(HttpHeaders.REFERER, nextUrl);
   ...
   private String setIdOnSellersUrl(String url, String internalId) {
      String sellersUrl = url;

      if (url.contains(SELLERS_URL_TOKEN)) {
         int x = url.indexOf(SELLERS_URL_TOKEN) + SELLERS_URL_TOKEN.length();
         int y = url.indexOf('/', x);

         sellersUrl = url.replace(url.substring(x, y), internalId);
      }
