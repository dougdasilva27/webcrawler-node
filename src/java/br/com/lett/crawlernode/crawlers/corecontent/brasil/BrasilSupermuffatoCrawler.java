 ...
 * 
 * 1) For this crawler, we have one URL for one sku, but if has more then on sku in url, this
 * crawler was prepared. 2) There is no stock information for skus in this ecommerce by the time
 * this crawler was made. 3) There is no marketplace in this ecommerce by the time this crawler was
 * made. 4) The sku page identification is done simply looking for an specific html element. 5) If
 * the sku is unavailable, it's price is not displayed. 6) The price of sku, found in json script,
 * is wrong when the same is unavailable, then it is not crawled. 7) There is internalPid for skus
 * in this ecommerce. The internalPid is a number that is the same for all the variations of a given
 * sku. 8) The primary image is the first image on the secondary images. 9) Images are get in api
 * "http://www.supermuffato.com.br/produto/sku/" + internalId 10) In json with images has attributte
 * isMain(boolean), if this is true, the image is the primary image.
 * 
 * 
 * Examples: ex1 (available):
 * http://www.supermuffato.com.br/smartphone-motorola-moto-g4-4g-android-6-0-octa-core-16gb-camera-13mp-tela-5-5-pol-preto-1017828/p
...
public class BrasilSupermuffatoCrawler extends Crawler {
  ...
  private boolean isProductPage(Document document) {
    if (document.select(".prd-info-container").first() != null)
      return true;
    return false;
  }
  ...
  /**
   * To crawl both card payment options and bank slip price we fetch a page containing all the
   * installments for all cards. The URL of this page is in this format:
  ...
  private boolean isArrayOfMainImages(JSONArray innerImagesArray) { // Identify the inner array of images that are the primary image
    ...
    if (innerImage.has("IsMain")) {
      if (innerImage.getBoolean("IsMain"))
        return true;
    }
    return false;
  }
  ...
  private String getLargestImage(JSONArray innerImagesArray) {
    for (int i = 0; i < innerImagesArray.length(); i++) {
      ...
      if (innerImage.has("Path")) {
        ...
        if (imagePath.contains("-1000-1000/"))
          return imagePath;
      }
    }
  ...
  private JSONArray crawlJsonImagesFromAPI(Document document, String internalId) {
    ...
    if (internalId != null) {
      ...
      skuData = CrawlerUtils.stringToJsonArray(this.dataFetcher.get(session, request).getBody());
    }
