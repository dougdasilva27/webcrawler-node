...
public class BrasilPontoanimalCrawler extends Crawler {
   ...
   private Float scrapPrice(String pricesString) {
      if (pricesString.contains("em até")) {
         return CrawlerUtils.scrapFloatPriceFromString(pricesString.toLowerCase(), ',', "por apenas", "em até", session);
      }

      if (pricesString.contains("ou R$")) {
         return CrawlerUtils.scrapFloatPriceFromString(pricesString.toLowerCase(), ',', "por apenas", "ou r$", session);
      }
      ...
      if (pricesString != null && price != null) {
         ...
         if (pricesString.contains("OFF de")) {
            prices.setPriceFrom(MathUtils.normalizeTwoDecimalPlaces(
                  CrawlerUtils.scrapFloatPriceFromString(pricesString.toLowerCase(), ',', "off de", "por apenas", session).doubleValue()));
         }

         prices.setBankTicketPrice(MathUtils.normalizeTwoDecimalPlaces(
               CrawlerUtils.scrapFloatPriceFromString(pricesString.toLowerCase(), ',', "ou r$", "no boleto", session).doubleValue()));
