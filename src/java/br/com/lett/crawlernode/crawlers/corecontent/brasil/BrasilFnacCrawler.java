...
public class BrasilFnacCrawler extends Crawler {
		...
		if (isProductPage(this.session.getOriginalURL(), doc)) {
			...
			for (int i = 0; i < skus.length(); i++) {
				...
				if (sku.has("bestPriceFormated") && available) {
					price = Float.parseFloat(sku.getString("bestPriceFormated").replaceAll("[^0-9,]+", "")
	...
	private Prices crawlPrices(Document doc, Float price) {
		...
		if (!installmentElements.isEmpty()) {
			...
			Float firstInstallmentPrice = Float.parseFloat(firstInstallmentElement.text().trim()
			...
			for (int i = 1; i < installmentElements.size(); i++) { // the first one is always the cash
				...
				if (installmentPriceElement != null) {
					installmentPrice = Float.parseFloat(installmentPriceElement.text().trim()
							.replaceAll("[^0-9,]+", "").replaceAll("\\.", "").replaceAll(",", "."));
				}
	 ...
	 * "length":45.0000, "weight":100.0000, "width":31.0000 }, "unitMultiplier":1.0000,
	 * "rewardValue":0 } ] }
	 * 
	 * @return
	 */
