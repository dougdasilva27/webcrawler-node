...
/**
 * 1) Only one page per sku. 2) No variations of skus in a page. 3) No voltage, size or any other
 * selector. 4) InternalId is crawled from a json object embedded inside the html code. this json
 * object is located inside a <script> html tag and is called skuJson_0.
 * 
 * @author samirleao
 *
 */
public class BrasilFnacCrawler extends Crawler {
  ...
  private boolean isProductPage(String url, Document document) {
    return (document.select(".x-product-details").first() != null);
  }
  ...
  private Prices crawlPrices(Document doc, Float price) {
    ...
    if (!installmentElements.isEmpty()) {
      // the first installment is a different html element
      Element firstInstallmentElement = installmentElements.get(0);
      Float firstInstallmentPrice =
          Float.parseFloat(firstInstallmentElement.text().trim().replaceAll("[^0-9,]+", "").replaceAll("\\.", "").replaceAll(",", "."));
      ...
      for (int i = 1; i < installmentElements.size(); i++) { // the first one is always the cash
                                                             // price (a vista)
        ...
        if (installmentNumberElement != null) {
          ...
          if (installmentText != null && !installmentText.isEmpty()) {
            installmentNumber = Integer.parseInt(MathUtils.parseNumbers(installmentText).get(0));
          }
        }
        if (installmentPriceElement != null) {
          installmentPrice =
              Float.parseFloat(installmentPriceElement.text().trim().replaceAll("[^0-9,]+", "").replaceAll("\\.", "").replaceAll(",", "."));
        }
  ...
  private String crawlName(JSONObject fullSkuJson, JSONObject skuJson) {
    ...
    // name between the two. Until the first version of this crawler, the ecommerce has no pages
    // with
    // more than one product to select. It's expected that with this approach, if the website starts
    // to
    ...
    if (nameFullSkuJson != null && nameSkuJson != null) {
      if (nameFullSkuJson.length() >= nameSkuJson.length())
        return nameFullSkuJson;
    }
    return nameSkuJson;
  }

  /**
   * Get the script having a json with the availability information. eg:
   * 
   * { "productId":692013, "name":"CASE LOGIC WMBP 115.01 MOCHILA PARA NOTEBOOK 15\" - PRETA",
   * "salesChannel":"1", "available":true, "displayMode":"especificacao", "dimensions":[ ],
   * "dimensionsInputType":{ }, "dimensionsMap":{ }, "skus":[ { "sku":7066198, "skuname":"CASE LOGIC
   * WMBP 115.01 MOCHILA PARA NOTEBOOK 15\" -", "dimensions":{ }, "available":true,
   * "availablequantity":5,
   * "cacheVersionUsedToCallCheckout":"596391fe885f5c0f09602ff20649b314_geral:6768E1225949CBC6E62EFD9EBBB3BF7B",
   * "listPriceFormated":"R$ 169,00", "listPrice":16900, "taxFormated":"R$ 0,00", "taxAsInt":0,
   * "bestPriceFormated":"R$ 152,10", "bestPrice":15210, "installments":3, "installmentsValue":5070,
   ...
   * "sellerId":"1", "seller":"fnac", "measures":{ "cubicweight":13.0781, "height":45.0000,
   * "length":45.0000, "weight":100.0000, "width":31.0000 }, "unitMultiplier":1.0000, "rewardValue":0
   * } ] }
   * 
   * @return
   */
