...
public class BrasilAmoedoCrawler extends Crawler {
	...
	private Float crawlMainPagePrice(Document document) {
		...
		if (specialPrice != null) {
			...
			if (regularPrice != null) {
				price = Float.parseFloat(regularPrice.text().toString().replaceAll("[^0-9,]+", "")
						.replaceAll("\\.", "").replaceAll(",", "."));
			} else {
	...
	private boolean crawlAvailability(Document document) {
		return document.select(".btesgotado").first() == null
				&& document.select(".availability.out-of-stock").first() == null;
	}
	...
	private Prices crawlPrices(Document doc, Float price) {
		...
		if (price != null) {
			...
			if (installments != null) {
				Integer installment =
						Integer.parseInt(installments.ownText().replaceAll("[^0-9]", "").trim());
