...
public class BrasilDellCrawler extends Crawler {
      ...
      if (infoJson.has("ProductMicroItems")) {
         ...
         for (Object o : productsArray) {
            ...
            if (productJson.has("Mpn") && productJson.has("Url")) {
               ...
               Request request = RequestBuilder.create().setUrl(newUrl).setCookies(cookies).build();
               Document newDoc = hasVariations ? Jsoup.parse(this.dataFetcher.get(session, request).getBody()) : doc;
   ...
   public Product extractProduct(Document doc, String internalId, String url) throws Exception {
      ...
      if (isProductPage(doc)) {
         ...
         RatingsReviews rating = crawlRatingReview(doc);
         ...
         product = ProductBuilder.create().setUrl(url).setInternalId(internalId).setName(name).setPrice(price).setPrices(prices).setAvailable(available)
               ...
               .setPrimaryImage(primaryImage).setSecondaryImages(secondaryImages).setDescription(description).setRatingReviews(rating).build();
   ...
   private RatingsReviews crawlRatingReview(Document doc) {
      ...
      if (infoJson.has("ProductMicroItems")) {
         ...
         for (Object o : productsArray) {
            ...
            if (productJson.has("Mpn") && productJson.has("Url")) {
               ...
               ratingReviews = crawlRating(internalId, newDoc);
            }
         }
      }
