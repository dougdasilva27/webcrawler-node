...
class BrasilSuperadegaCrawler(session: Session) : VTEXOldScraper(session) {
   ...
   fun getTotalNumOfRatingsFromYourViews(doc: Document): Int {
      val totalRatingElement = doc.selectFirst("#yv-quickreview-more")
   ...
   fun getTotalAvgRatingFromYourViews(docRating: Document): Double {
      ...
      if (rating?.attr("content") != null) {
         return rating.attr("content").toDouble()
      } else if (starsSum > 0) {
         return starsSum.toDouble() / starsCount
      }
      return 0.0
   }
