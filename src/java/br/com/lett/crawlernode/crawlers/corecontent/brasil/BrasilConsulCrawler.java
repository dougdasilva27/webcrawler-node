...
public class BrasilConsulCrawler extends Crawler {
   ...
   * containing the payment options we have one table for each card brands. On the examples used
   * during this crawler development, all the tables were the same, but we want to make robust
   ...
   * evalue to apply as discount. It was observed that all the discounts in bank slip are 5%. The html
   * element is ".flag.-cns--desconto-5--boleto". If this element is found, then we apply the 5%
   * discount on the best price, otherwise we consider the bestPrice as being the bank slip price.
   * 
   * @param document
   * @param skuInformationJson
   * @return
