 ...
 * http://www.soubarato.com.br/produto/6603120/ar-condicionado-de-janela-springer-duo-7.500-btus-frio-mecanico
 * ex2 (unavailable):
 * http://www.soubarato.com.br/produto/125911511/smartphone-samsung-galaxy-s7-edge-desbloqueado-android-6.0-tela-5-5-32gb-4g-12mp-prata
 *
...
public class BrasilSoubaratoCrawler extends Crawler {
  ...
  public List<Product> extractInformation(Document doc) throws Exception {
    ...
    if (isProductPage(this.session.getOriginalURL())) {
      ...
      /*
       * ********************************************************* crawling data common to both the cases
       * of product page *
       ***********************************************************/
  ...
  private boolean crawlAvailability(Document doc) {
    ...
    for (Element e : avaibilityElements) {
      ...
      if (symbol.equals("N"))
        return true; // Symbol N means new product
    }
    ...
    if (!isCategorySpecial) {
      ...
      if (elementFotoSecundaria.size() > 1) {
        for (int i = 1; i < elementFotoSecundaria.size(); i++) { // starts with index 1 because de primary image is the first image
          ...
          if (!primaryImage.equals(secondaryImagesTemp))
            secondaryImagesArray.put(secondaryImagesTemp); // identify if the image is the primary image
        }
      }
