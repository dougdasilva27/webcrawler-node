 ...
 * all the variations of a given sku. 7) The primary image is the first image on the secondary
 * images. 8) Variations of skus are not crawled if the variation is unavailable because it is not
 * displayed for the user, except if there is a html element voltage, because variations of voltage
 * are displayed for the user even though unavailable. 9) The url of the primary images are changed
 * to bigger dimensions manually.
 * 
 * Price crawling notes: 1) For the bank slip price we use a global discount rate of 5%, as we
 * observed that this value was the default discount on the ecommerce. This discount rate is applyed
 * on the lowest installment price (menor preço a prazo). After we apply the discount on the base
 * price, we use round the result to the uppermost decimal value, because we observed that thats the
 * rounding policy used in this ecommerce.
 * 
 * 2) For the card payment options we fetch a page containing all the payment options. This page
 * also includes the bank slip price, but this one is inconsistent to that displayed on the sku main
 * page, and that's why we doesn't crawl it.
 * 
 * Examples: ex1 (available):
 * http://www.eletronicasantana.com.br/ar-condicionado-portatil-9000-btus-quente-frio--acp09qf-%E2%80%93-agratto/p
 * ex2 (unavailable):
 * http://www.eletronicasantana.com.br/forno-de-micro-ondas-32-l-900-w-branco-st652wruk---panasonic-08736/p
 *
 *
 **************************************************************************************************************************************/
...
public class BrasilEletronicasantanaCrawler extends Crawler {
  ...
  /**
   * To crawl both card payment options we fetch a page containing all the installments for all cards.
   * The URL of this page is in this format:
   * "http://www.supermuffato.com.br/productotherpaymentsystems/internalId"
   * 
   * When the sku is unavailable we doesn't crawl the prices, because they are not displayed for the
   * user.
   * 
  ...
  /**
   * For the bank slip price we use a global discount rate of 5%, as we observed that this value was
   * the default discount on the ecommerce. This discount rate is applyed on the lowest installment
   * price (menor preço a prazo). After we apply the discount on the base price, we use round the
   * result to the uppermost decimal value, because we observed that thats the rounding policy used in
   * this ecommerce.
   * 
   * @param skuInformationJson
   * @return
    ...
    for (int i = 1; i < lines.size(); i++) { // the first is the table header
      ...
      if (installmentTextElement != null) {
        ...
        if (installmentText.contains(Card.VISA.toString()))
          return Card.VISA;
        if (installmentText.contains(Card.AMEX.toString()) || installmentText.contains("american express"))
          return Card.AMEX;
        ...
        if (installmentText.contains(Card.ELO.toString()))
          return Card.ELO;
        if (installmentText.contains(Card.AURA.toString()))
          return Card.AURA;
        if (installmentText.contains(Card.DISCOVER.toString()))
  ...
  private String crawlDescription(Document document) {
    ...
    if (embalagemElement != null)
      description = description + embalagemElement.html();

    return description;
  }

  /**
   * Search for a particular script html element that contains a JSONArray with all the information we
   * need about the skus on the page. This method will parse only the JSONArray inside this script,
   * and ignores the remaining information inside the script tag.
   * 
   * e.g: var skuJson_0 = {"productId":9000346,"name":"kit Conversor Digital SC-1001 Pix + Antena
   * Externa Cromus CR2100","salesChannel":"1",
   * "available":true,"displayMode":"especificacao","dimensions":[],"dimensionsInputType":{},
   * "dimensionsMap":{},"skus": [ {"sku":9000352, "skuname":"kit Conversor Digital SC-1001 Pix +
   * Antena Externa Cromus CR2100","dimensions":{}, "available":true, "availablequantity":99999,
   * "cacheVersionUsedToCallCheckout":"b945c45336540f7aa63e5810464143a1_geral:D99E18D9FD8A98F5A3C7FC0A5A871ED1",
   * "listPriceFormated":"R$ 189,90", "listPrice":18990, "taxFormated":"R$ 0,00", "taxAsInt":0,
   * "bestPriceFormated":"R$ 169,90", "bestPrice":16990,"installments":6,"installmentsValue":2831,
   ...
   * };CATALOG_SDK.setProductWithVariationsCache(skuJson_0.productId, skuJson_0); var skuJson =
   * skuJson_0;
   * 
    ...
    for (Element tag : scriptTags) {
      for (DataNode node : tag.dataNodes()) {
        if (tag.html().trim().startsWith("var skuJson_0 = ")) {
          ...
          String jsonString = nodeWholeData.split(Pattern.quote("var skuJson_0 = "))[1]
