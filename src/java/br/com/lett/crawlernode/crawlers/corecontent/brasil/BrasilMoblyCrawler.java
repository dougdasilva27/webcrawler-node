...
public class BrasilMoblyCrawler extends Crawler {
		...
		if (isProductPage(doc)) {
			...
			if (!skus.isEmpty()) {
				...
				for (Element sku : skus) {
					...
					// Sku Json
					JSONObject jsonSku =
							this.assembleJsonProduct(internalID, internalPid, jsonProductsApi, jsonProduct);
				...
				// Sku Json
				JSONObject jsonSku =
						this.assembleJsonProduct(internalID, internalPid, jsonProductApi, jsonProduct);
	...
	private String crawlInternalIdSingleProduct(Document document) {
		String internalId = null;
		Element internalIdElement =
				document.select(".add-wishlistsel-product-move-to-wishlist").first();
	...
	private JSONObject fetchSkuInformation(String internalIDS) {
		String url =
				"https://secure.mobly.com.br/api/catalog/price/hash/34ad996ba25a3c5b5c352406f932a3e31e0a45b7/?skus="
						+ internalIDS;
	...
	private JSONObject assembleJsonProduct(String internalID, String internalPid, JSONObject jsonPage,
		...
		if (jsonPage.has("priceStore")) {
			...
			if (jsonTemp.has(internalPid)) {
				...
				/**
				 * in which case the api prices did not return the correct product, so the json is caught in
				 * a javascript script in hmtl
				 */
	...
	private Map<String, Float> crawlMarketplace(Document document, JSONObject jsonSku) {
		...
		if (marketplace != null) {
			...
			if (jsonSku.has("Available")) {
				if (jsonSku.getBoolean("Available")) {
					if (jsonSku.has("Price")) {
						Float price = Float.parseFloat(jsonSku.getString("Price").replaceAll("[^0-9,]+", "")
			...
			if (jsonSku.has("Available")) {
				if (jsonSku.getBoolean("Available")) {
					if (jsonSku.has("Price")) {
						price = Float.parseFloat(jsonSku.getString("Price").replaceAll("[^0-9,]+", "")
								.replaceAll("\\.", "").replaceAll(",", "."));
