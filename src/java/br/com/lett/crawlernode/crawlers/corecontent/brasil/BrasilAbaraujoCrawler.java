...
public class BrasilAbaraujoCrawler extends Crawler {

  public BrasilAbaraujoCrawler(Session session) {
    super(session);
  }

  private static final String SELLER_FULL_NAME = "Ab Araujo";
  ...
  public List<Product> extractInformation(Document doc) throws Exception {
    ...
    if (isProductPage(doc)) {
      ...
      JSONObject variationJson =
          CrawlerUtils.selectJsonFromHtml(doc, "script[type=\"text/x-magento-init\"]",
              "\"#product_addtocart_form\": ", ",", false, true);
      JSONObject stockJson = CrawlerUtils.selectJsonFromHtml(doc,
          "script[type=\"text/x-magento-init\"]", "\".input-text.qty\": ", "}", false, true);
      JSONObject imagesJson =
          CrawlerUtils.selectJsonFromHtml(doc, "script[type=\"text/x-magento-init\"]",
              "\"[data-gallery-role=gallery-placeholder]\": ", "}", false, true);
      JSONArray imagesArr = scrapImages(imagesJson);

      String internalId = CrawlerUtils.scrapStringSimpleInfoByAttribute(doc,
          "#product_addtocart_form > input[type=hidden]:nth-child(1)", "value");
      String internalPid = internalId;
      String name = CrawlerUtils.scrapStringSimpleInfo(doc, "h1.page-title", false);
      CategoryCollection categories =
          CrawlerUtils.crawlCategories(doc, ".breadcrumbs > ul > li:not(:last-child)", true);
      ...
      String description = CrawlerUtils.scrapElementsDescription(doc, Arrays.asList(
          ...
          ".product.info > div > div:not(#tab-label-reviews):not(#reviews):not(#tab-label-questions):not(#questions)"));
      ...
      Product product =
          ProductBuilder.create().setUrl(session.getOriginalURL()).setInternalId(internalId)
              .setInternalPid(internalPid).setName(name).setCategory1(categories.getCategory(0))
              .setCategory2(categories.getCategory(1)).setCategory3(categories.getCategory(2))
              .setPrimaryImage(primaryImage).setSecondaryImages(secondaryImages)
              .setDescription(description).setOffers(offers).build();
      ...
      if (variationJson != null && !variationJson.keySet().isEmpty()) {
        ...
        products.addAll(buildVariationProducts(variationJson, product, imagesArr,
            extractStockFromJson(stockJson)));
  ...
  private List<Product> buildVariationProducts(JSONObject json, Product product,
      JSONArray imagesArr, Map<String, Integer> stocks)
    ...
    for (String key : json.keySet()) {
      if (json.get(key) instanceof JSONObject) {
        ...
        String varName = varCode != null && varLabel != null
            ? product.getName() + " - " + varCode + " " + varLabel
            : product.getName();
            ...
            if (variationJson.has("products")
              ...
              for (Object obj : variationJson.getJSONArray("products")) {
                if (obj instanceof String) {
                  variations.put((String) obj,
                      new JSONObject().put("label", label).put("code", code));
                }
              }
            }
          }
  ...
  private boolean scrapAvailability(Document doc) {
    return doc.selectFirst(".stock.available") != null;
  }

  private JSONArray scrapImages(JSONObject json) {
    JSONArray secondaryImagesArray = new JSONArray();

    if (json.has("mage/gallery/gallery")
        && json.get("mage/gallery/gallery") instanceof JSONObject) {
      json = json.getJSONObject("mage/gallery/gallery");

      JSONArray images =
          json.has("data") && json.get("data") instanceof JSONArray ? json.getJSONArray("data")
              : new JSONArray();
  ...
  private Offers scrapOffers(Document doc) throws MalformedPricingException, OfferException {
    ...
    Double priceFrom =
        CrawlerUtils.scrapDoublePriceFromHtml(doc, ".old-price .price", null, false, ',', session);
    Double spotlightPrice = CrawlerUtils.scrapDoublePriceFromHtml(doc, ".price-container .price",
        null, false, ',', session);
    Pricing pricing = scrapPricing(doc, priceFrom, spotlightPrice);
    List<String> sales = scrapSales(priceFrom, spotlightPrice);

    offers.add(OfferBuilder.create().setUseSlugNameAsInternalSellerId(true)
        .setSellerFullName(SELLER_FULL_NAME).setMainPagePosition(1).setIsBuybox(false)
        .setIsMainRetailer(true).setPricing(pricing).setSales(sales).build());
  ...
  private List<String> scrapSales(Double priceFrom, Double spotlightPrice) {
    ...
    if (priceFrom != null && spotlightPrice != null) {
      if (priceFrom > spotlightPrice) {
        Double discount =
            MathUtils.normalizeTwoDecimalPlaces((spotlightPrice / priceFrom) - 1) * 100;
  ...
  private Pricing scrapPricing(Document doc, Double priceFrom, Double spotlightPrice)
      throws MalformedPricingException {

    CreditCards creditCards = scrapCreditCards(doc, spotlightPrice);
    BankSlip bankSlip = BankSlipBuilder.create().setFinalPrice(spotlightPrice).build();

    return PricingBuilder.create().setPriceFrom(priceFrom).setSpotlightPrice(spotlightPrice)
        .setCreditCards(creditCards).setBankSlip(bankSlip).build();
  ...
  private CreditCards scrapCreditCards(Document doc, Double spotlightPrice)
    ...
    for (String card : cards) {
      creditCards.add(CreditCardBuilder.create().setBrand(card).setInstallments(installments)
  ...
  public Installments scrapInstallments(Document doc, Double spotlightPrice)
      throws MalformedPricingException {
  ...
  private Offers scrapOfferswithVariation(JSONObject json)
      throws MalformedPricingException, OfferException {
    Offers offers = new Offers();
    ...
    if (spotlightPriceV.equals(priceFromV)) {
      priceFromV = null;
    }

    Pricing pricing = scrapPricingForProductsWithVariation(json, priceFromV, spotlightPriceV);
    List<String> sales = scrapSalesWithVariation(priceFromV, spotlightPriceV);
  ...
  private List<String> scrapSalesWithVariation(Double priceFromV, Double spotlightPriceV) {
    List<String> sales = new ArrayList<>();

    if (priceFromV != null && spotlightPriceV != null) {
      if (priceFromV > spotlightPriceV) {
        Double discount =
            MathUtils.normalizeTwoDecimalPlaces((spotlightPriceV / priceFromV) - 1) * 100;
        sales.add(Integer.toString(discount.intValue()).replace("-", "- ".replace(".0", "")) + "%");
  ...
  private Pricing scrapPricingForProductsWithVariation(JSONObject variationJson, Double priceFromV,
      Double spotlightPriceV) throws MalformedPricingException {

    CreditCards creditCardsV =
        scrapCreditCardsForProductsWithVariation(variationJson, spotlightPriceV);
    BankSlip bankSlipV = BankSlipBuilder.create().setFinalPrice(spotlightPriceV).build();

    return PricingBuilder.create().setPriceFrom(priceFromV).setSpotlightPrice(spotlightPriceV)
        .setCreditCards(creditCardsV).setBankSlip(bankSlipV).build();
  }

  private CreditCards scrapCreditCardsForProductsWithVariation(JSONObject variationJson,
      Double spotlightPriceV) throws MalformedPricingException {
    CreditCards creditCardsV = new CreditCards();

    Installments installments =
        scrapInstallmentsForProductsWithVariation(variationJson, spotlightPriceV);
    if (installments.getInstallments().isEmpty()) {
      installments.add(InstallmentBuilder.create().setInstallmentNumber(1)
          .setInstallmentPrice(spotlightPriceV).build());
    }

    for (String card : cards) {
      creditCardsV.add(CreditCardBuilder.create().setBrand(card).setInstallments(installments)
          .setIsShopCard(false).build());
    }
  ...
  public Installments scrapInstallmentsForProductsWithVariation(JSONObject variationJson,
      Double spotlightPriceV) throws MalformedPricingException {
    Installments installments = new Installments();
