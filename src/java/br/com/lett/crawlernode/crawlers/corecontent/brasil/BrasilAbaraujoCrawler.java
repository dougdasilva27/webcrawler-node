...
public class BrasilAbaraujoCrawler extends Crawler {
	...
	public List<Product> extractInformation(Document doc) throws Exception {
		...
		if (isProductPage(doc)) {
			...
			JSONObject variationJson = CrawlerUtils.selectJsonFromHtml(doc, "script[type=\"text/x-magento-init\"]",
					"\"#product_addtocart_form\": ", ",", false, true);
			JSONObject stockJson = CrawlerUtils.selectJsonFromHtml(doc, "script[type=\"text/x-magento-init\"]",
					"\".input-text.qty\": ", "}", false, true);
			JSONObject imagesJson = CrawlerUtils.selectJsonFromHtml(doc, "script[type=\"text/x-magento-init\"]",
					"\"[data-gallery-role=gallery-placeholder]\": ", "}", false, true);
			JSONArray imagesArr = scrapImages(imagesJson);

			String internalId = CrawlerUtils.scrapStringSimpleInfoByAttribute(doc, "#product_addtocart_form > input[type=hidden]:nth-child(1)",
					"value");
			String internalPid = internalId;
			String name = CrawlerUtils.scrapStringSimpleInfo(doc, "h1.page-title", false);
			CategoryCollection categories = CrawlerUtils.crawlCategories(doc, ".breadcrumbs > ul > li:not(:last-child)",
					true);
			...
			String description = CrawlerUtils.scrapElementsDescription(doc, Arrays.asList("[itemprop=description]",
			...
			if (variationJson != null && !variationJson.keySet().isEmpty()) {
				...
				products.addAll(
						buildVariationProducts(variationJson, product, imagesArr, extractStockFromJson(stockJson)));
	...
	private List<Product> buildVariationProducts(JSONObject json, Product product, JSONArray imagesArr,
			Map<String, Integer> stocks) throws MalformedPricingException, OfferException {
		List<Product> products = new ArrayList<>();
	...
	private JSONArray scrapImages(JSONObject json) {
		...
		if (json.has("mage/gallery/gallery") && json.get("mage/gallery/gallery") instanceof JSONObject) {
			...
			JSONArray images = json.has("data") && json.get("data") instanceof JSONArray ? json.getJSONArray("data")
					: new JSONArray();
	...
	private Offers scrapOffers(Document doc) throws MalformedPricingException, OfferException {
		...
		Double spotlightPrice = CrawlerUtils.scrapDoublePriceFromHtml(doc, ".price-container .price", null, false, ',',
				session);
		Pricing pricing = scrapPricing(doc, priceFrom, spotlightPrice);
		List<String> sales = scrapSales(priceFrom, spotlightPrice);

		offers.add(OfferBuilder.create().setUseSlugNameAsInternalSellerId(true).setSellerFullName(SELLER_FULL_NAME)
				.setMainPagePosition(1).setIsBuybox(false).setIsMainRetailer(true).setPricing(pricing).setSales(sales)
				.build());
	...
	private CreditCards scrapCreditCards(Document doc, Double spotlightPrice) throws MalformedPricingException {
		...
		if (installments.getInstallments().isEmpty()) {
			installments.add(
					InstallmentBuilder.create().setInstallmentNumber(1).setInstallmentPrice(spotlightPrice).build());
		}

		for (String card : cards) {
			creditCards.add(CreditCardBuilder.create().setBrand(card).setInstallments(installments).setIsShopCard(false)
					.build());
		}
	...
	public Installments scrapInstallments(Document doc, Double spotlightPrice) throws MalformedPricingException {
		...
		installments
				.add(InstallmentBuilder.create().setInstallmentNumber(1).setInstallmentPrice(spotlightPrice).build());
	...
	private CreditCards scrapCreditCardsForProductsWithVariation(JSONObject variationJson, Double spotlightPriceV)
			throws MalformedPricingException {
		...
		if (installments.getInstallments().isEmpty()) {
			installments.add(
					InstallmentBuilder.create().setInstallmentNumber(1).setInstallmentPrice(spotlightPriceV).build());
		}
	...
	public Installments scrapInstallmentsForProductsWithVariation(JSONObject variationJson, Double spotlightPriceV)
			throws MalformedPricingException {
		Installments installments = new Installments();

		installments
				.add(InstallmentBuilder.create().setInstallmentNumber(1).setInstallmentPrice(spotlightPriceV).build());
