 ...
 * Date: 08/12/2016
 * 
 * 1) In time crawler was made, there no product unnavailable. 2) There is no bank slip (boleto
 * bancario) payment option. 3) There is no installments for card payment. So we only have 1x
 * payment, and to this value we use the cash price crawled from the sku page. (nao existe divisao
 * no cartao de credito).
 * 
 * 4) In this crawler is requires use webdriver 5) Bogota was city choose 6) Is required some
 * cookies that are only getted via webdriver
 * 
...
public class ColombiaCarullaCrawler extends Crawler {
  ...
  /**
   * There is no bankSlip price. In this market has no installments informations
   * 
   * @param doc
   * @param price
   * @return
