  ...
  public List<Product> crawlProductsNewWay() {
    ...
    if (productAPIJSON.length() > 0) {
      ...
      for (int i = 0; i < arraySkus.length(); i++) {
        ...
        JSONObject skuAPIJSON =
            BrasilFastshopCrawlerUtils.crawlApiJSON(variationJson.has("partNumber") ? variationJson.getString("partNumber") : null, session, cookies);
        String primaryImage = crawlPrimaryImage(skuAPIJSON);
        String name = crawlName(skuAPIJSON, variationJson);
