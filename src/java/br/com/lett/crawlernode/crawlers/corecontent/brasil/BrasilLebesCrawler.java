...
public class BrasilLebesCrawler extends Crawler {
	 ...
	 * e.g:
	 * 
	 * Nº de Parcelas Valor de cada parcela Visa à vista R$ 479,90 Visa 2 vezes sem juros R$ 239,95
	 * Visa 3 vezes sem juros R$ 159,96 Visa 4 vezes sem juros R$ 119,97 Visa 5 vezes sem juros R$
	 * 95,98 Visa 6 vezes sem juros R$ 79,98 Visa 7 vezes sem juros R$ 68,55 Visa 8 vezes sem juros R$
	 * 59,98 Visa 9 vezes sem juros R$ 53,32 Visa 10 vezes sem juros R$ 47,99 Visa 11 vezes com juros
	 * R$ 46,26 Visa 12 vezes com juros R$ 42,61
	 *
	 * @param table
	 * @return
	 */
	 ...
	 * 
	 * For the calculations we round the final number to the lower bound, as observed on the
	 * ecommerce.
	 * 
	 * @param document
	 * @param jsonSku
	...
	private String crawlDescription(Document document) {
		...
		if (ean != null && detalheNagem != null && !detalheNagem.html().trim().isEmpty()) {
			description.append(CrawlerUtils.crawlDescriptionFromFlixMedia("11947", ean, session));
		}
	...
	private String crawlEan(Document doc) {
		...
		JSONObject json = CrawlerUtils.selectJsonFromHtml(doc, "script", "vtex.events.addData(", ");");
	...
	/**
	 * Get the json containing all skus info. In case we can't find the json on the already loaded
	 * html from the sku page, we try to fetch this json from an API.
	 * 
	 * @param document
	 * @return
	 */
