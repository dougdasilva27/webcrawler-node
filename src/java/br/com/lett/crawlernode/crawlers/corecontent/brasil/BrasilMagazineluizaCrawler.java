...
public class BrasilMagazineluizaCrawler extends Crawler {
	...
	public List<Product> extractInformation(Document doc) throws Exception {
		...
		if(isProductPage(session.getOriginalURL(), doc)) {
			...
			boolean isNewSite = doc.select(".breadcrumb__title").first() != null;
			...
			if(isNewSite) {
				...
				if(singleProduct) {
					...
					for(int i = 0; i < skus.length(); i++) {
						...
						// InternalId
						String internalId = internalIdSecondPart != null ? internalIdSecondPart : internalIdSingleProduct;
	...
	private CategoryCollection crawlCategories(Document document) {
		...
		Elements elementCategories = document.select(".breadcrumb .breadcrumb__title a:not(.icon-home)");

		for (int i = 0; i < elementCategories.size()-1; i++) { // -1 because the last index is the product
