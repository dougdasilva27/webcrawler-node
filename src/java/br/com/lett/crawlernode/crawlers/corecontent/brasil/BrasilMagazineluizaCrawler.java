...
public class BrasilMagazineluizaCrawler extends Crawler {
	...
	public List<Product> extractInformation(Document doc) throws Exception {
		...
		if(isProductPage(session.getOriginalURL(), doc)) {
			...
			String internalId = crawlInternalIdSingleProduct(skuJsonInfo);
	 ...
	 * @return
	 */
	private String crawlInternalIdSingleProduct(JSONObject skuJson) {
	...
	private Marketplace crawlMarketPlace(Document doc, JSONObject skuInfo) {
		...
		if (marketplaceName != null) {
			...
			if (!sellerName.equalsIgnoreCase(SELLER_NAME) && (skuInfo == null || (skuInfo.has("salePrice") && skuInfo.get("salePrice") instanceof Double))) {
	...
	private Prices crawlPrices(Float price, Document doc, JSONObject skuInfo){
		...
		if(price != null){
			...
			if(skuInfo.has("cashPrice")) {
				Double aVista = skuInfo.getDouble("cashPrice");
