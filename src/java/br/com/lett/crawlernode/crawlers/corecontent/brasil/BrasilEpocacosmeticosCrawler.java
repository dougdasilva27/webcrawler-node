...
public class BrasilEpocacosmeticosCrawler extends Crawler {
  ...
  public List<Product> extractInformation(Document doc) throws Exception {
    ...
    if (isProductPage(doc)) {
      ...
      System.out.println(skuJson.toString());
      
      // String description = crawlDescription(doc);
      //
      // // sku data in json
      // JSONArray arraySkus = skuJson != null && skuJson.has("skus") ? skuJson.getJSONArray("skus") : new
      // JSONArray();
      //
      ...
      // marketplaceMap.get(MAIN_SELLER_NAME_LOWER) : new Prices();
      // Float price = vtexUtil.crawlMainPagePrice(prices);
      // Integer stock = vtexUtil.crawlStock(apiJSON);
      //
      // // Creating the product
      // Product product =
      // ProductBuilder.create().setUrl(session.getOriginalURL()).setInternalId(internalId).setInternalPid(internalPid).setName(name)
