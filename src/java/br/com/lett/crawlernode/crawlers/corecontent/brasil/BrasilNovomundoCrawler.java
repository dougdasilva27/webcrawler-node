...
public class BrasilNovomundoCrawler extends Crawler {
  ...
  private boolean crawlAvailability(JSONObject productInfoJSON) {
    ...
    if (availabilityFromJson && isDefaultSeller)
      return true;
    return false;
  }
  ...
  private Float crawlPrice(JSONObject productInfoJSON) {
    ...
    if (price != null && Float.compare(price, 0.0f) == 0)
      price = null;
      ...
      if (seller.has("SellerId")) {
        if (seller.getString("SellerId").equals("1")) {
          if (seller.has("AvailableQuantity"))
            return seller.getInt("AvailableQuantity");
        }
      }
    }
  ...
  private Prices crawlPricesForMarketplaces(Float price) {
