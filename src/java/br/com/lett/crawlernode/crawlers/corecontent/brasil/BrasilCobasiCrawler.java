...
public class BrasilCobasiCrawler extends Crawler {
  ...
  public List<Product> extractInformation(Document doc) throws Exception {
    ...
    if (isProductPage(doc)) {
      ...
      for (int i = 0; i < arraySkus.length(); i++) {
        ...
        primaryImage = (primaryImage == null) ? crawlPrimaryImage(jsonProduct) : primaryImage; // because this markets only show images of the first
                                                                                               // product
        secondaryImages = (secondaryImages == null) ? crawlSecondaryImages(jsonProduct) : secondaryImages; // because this markets only show images of
                                                                                                           // the first product
  ...
  private Prices crawlPrices(String internalId, Float price, JSONObject jsonSku) {
    ...
    if (price != null) {
      String url = HOME_PAGE + "productotherpaymentsystems/" + internalId;
