 ...
 *
 */
public class BrasilShopfatoRatingReviewCrawler extends RatingReviewCrawler {

	public BrasilShopfatoRatingReviewCrawler(Session session) {
		super(session);
	}
	...
	/**
	 * Api Ratings Url: http://www.shopfato.com.br/userreview Ex payload:
	...
	private Document crawlPageRatings(String url, String internalPid) {
		...
		String response = POSTFetcher.fetchPagePOSTWithHeaders("http://www.shopfato.com.br/userreview",
				session, payload, cookies, 1, headers);
	...
	private List<String> crawlIdList(Document doc) {
		...
		if (elementInternalId != null) {
			String[] ids = elementInternalId.attr("value").trim().split(",");
