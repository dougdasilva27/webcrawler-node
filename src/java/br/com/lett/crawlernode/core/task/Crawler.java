				...
				previousProcessedProduct != null) 
		{
			...
			if (previousProcessedProduct == null) { 
				...
				if (newProcessedProduct != null) {
					...
					Logging.printLogDebug(logger, session, "Processed id returned from persistence: " + id);
					
					// append the processed id on mongo task and end
					if (id != null) {
						Logging.printLogDebug(logger, session, "Appending processed id " + id + " on mongo task...");
