...
public class SaopauloSubmarinoCrawler extends Crawler {
	...
	public List<Product> extractInformation(Document doc) {
		...
		if(session.getUrl().startsWith("http://www.submarino.com.br/produto/") && !session.getUrl().contains("?loja=")) {
			Logging.printLogDebug(logger, session, "Product page identified: " + this.session.getUrl());
			...
			if (elementsProductOptions.size() == 0) { // não possui variações
				...
				Logging.printLogDebug(logger, "Capturando informações de produto sem variações...");
				...
				Logging.printLogDebug(logger, "Capturando informações de produto com variações...");
	...
	private ArrayList<Document> fetchMarketplaceInfoArray(Elements elementsProductOptions, String internalPid) {
		Logging.printLogDebug(logger, "Pegando páginas com informações de lojistas das variações...");
